...
vector<pair<string,string>>  LexicalAnalyzer::Analyse(string str){
    ...
    for(int index=0;index<str.size();index++){
        ...
        if(curr_state==S_ERROR ||curr_str.size()>16) {
            ...
            while(str[index]!=' '&&str[index]!='\n'&& str[index]!='\0'&&str[index]!=';'&&str[index]!='\t'){
                index++;
                curr_str+=str[index];
            }
            cout<< "Error in string:  "<< curr_str<<endl;
            curr_str.clear();
        }
    }
    if(!curr_str.empty()) {
    ...
    if(curr_state==S0) {
        if (prev_state == S0) {
            if(curr_str!=";")
                ...
                ret = make_pair("Semicolon", curr_str);
            }
        }
        if (prev_state == S_WHILE_FINAL || prev_state == S_DO_FINAL) {
            ret = make_pair("Key word", curr_str);
        }
        if (prev_state == S_W1 || prev_state == S_W2 || prev_state == S_W3 || prev_state == S_W4 ||
            prev_state == S_DO1 || prev_state == S_VAR) {
